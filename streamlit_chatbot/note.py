...
# In a real application, use st.secrets or environment variables (e.g., os.getenv("GEMINI_API_KEY"))
...
try:
    ...
    # Use a powerful model for complex roleplaying and instruction following
...
# This detailed prompt forces Gemini to act as a D&D 5e Dungeon Master.
...
4.  **Skill Checks & Rolls:**
    * For actions with uncertain outcomes, you will ask the player to make a relevant **Skill Check** or **Saving Throw** (e.g., "Make a DC 13 Perception check").
    * The player will tell you their **d20 roll result** and any **modifier** (e.g., "I rolled a 15 + 2 for a total of 17").
    * You then determine the outcome based on the Difficulty Class (DC) and describe the result.
5.  **Combat:**
    * When combat begins, you will ask for an **Initiative Roll** from the player. You will generate and list all initiatives.
    * You will track **HP, AC, and status** for all non-player entities. The player tracks their own.
    * The combat should proceed in rounds, with clear turn-order descriptions.
6.  **Pacing:** Keep the story moving. After each description, clearly state what the player's options or immediate challenges are, and then wait for their input.

### Initial Game State:
* **Player Character:** Assume the player will introduce their character (Name, Race, Class, Level 1). If they don't, politely ask them to create one before starting the adventure.
* **Starting Adventure:** Once the character is set, describe the initial setting and the first scene of the adventure.

**Your first response MUST be a friendly welcome, an introduction of your role as the DM, and an invitation for the player to describe or create their Level 1 character.**
"""

# --- 3. Streamlit Session State and Chat Management ---
...
if "chat_session" not in st.session_state:
    st.session_state.chat_session = model.start_chat(
        history=[],
        # This is the correct way to enforce the persona across the entire conversation
...
# Sidebar for controls and clear button
with st.sidebar:
    st.title("Game Controls")
    st.markdown("Use this panel to manage your session.")
    st.markdown("---")
    
    # Character Info (For Player to Manually Track - DM does not store this)
    st.subheader("Your Character Sheet")
    st.text_input("Character Name", "Bardic Bob")
    st.text_input("HP / AC", "10 / 14")
    
    # Clear Chat History
    if st.button("Start New Adventure / Clear Chat", type="primary"):
        st.session_state.messages = []
        # Re-initialize the chat session to reset the AI's memory and system instruction
        st.session_state.chat_session = model.start_chat(
            history=[],
            system_instruction=DND_SYSTEM_INSTRUCTION
        )
...
for message in st.session_state.messages:
    ...
    avatar = "👤" if message["role"] == "user" else "🔮" # Player is user, DM is assistant
    with st.chat_message(message["role"], avatar=avatar):
        st.markdown(message["content"])

# --- 5. Chat Input and Game Logic ---
if prompt := st.chat_input("Enter your character's action or response..."):
    ...
    with st.chat_message("assistant", avatar="🔮"):
        with st.spinner("The DM is consulting the scrolls..."):
            try:
                ...
                # Stream the response content to the screen
                response_text = st.write_stream(response)

                # 4. Add DM response to session history
...
# --- 1. Configuration & API Key Setup ---
# *** IMPORTANT: REPLACE THIS PLACEHOLDER WITH YOUR ACTUAL GEMINI API KEY ***
# RECOMMENDATION: For security, use st.secrets or environment variables (e.g., os.getenv("GEMINI_API_KEY"))
# DO NOT hardcode your key directly in your file in a production environment.
GOOGLE_API_KEY = "AIzaSyBtMGPg3oUK0K0akIwlZW2sGzTleED4vRw" 
...
try:
    ...
    # --- FIX APPLIED HERE ---
    # The 'system_instruction' is now passed when the GenerativeModel is created.
    # This correctly enforces the DM persona for the entire session.
    model = genai.GenerativeModel(
        'gemini-2.5-pro',
        system_instruction=DND_SYSTEM_INSTRUCTION # CORRECT PLACE
    ) 
except Exception as e:
    st.error(f"Error configuring Gemini API: {e}")
    st.stop()

# --- 3. Streamlit Session State and Chat Management (Simplified) ---
...
# The 'system_instruction' is no longer needed here, as it was set on the model object.
if "chat_session" not in st.session_state:
...
### Game Start Sequence (CRITICAL):
1.  **Welcome & Scene Setup:** Your absolute first response must be a friendly welcome and a request for the player to input their character details in the sidebar. Then, immediately transition to setting the scene by asking the player: **"Adventurer, with your details noted, do you wish to describe the first scene/location yourself, or shall I roll on the Random Encounters table to start the adventure?"** 2.  The adventure begins only after the player answers this question.
"""
...
st.info("⚔️ **Roll-Playing Rules:** When the DM asks for a roll (e.g., 'Make a DC 13 Perception check'), type your **total result** (e.g., 'I got an 18'). The DM will handle the outcome. **Start by filling out your character details in the sidebar!**")
    ...
    with st.chat_message("assistant", avatar="🔮"):
        ...
        with st.spinner("The DM is consulting the scrolls..."):
            try:
                ...
                # After streaming is complete, the full_response variable holds the clean text
